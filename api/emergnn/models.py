...
class EmerGNN(nn.Module):
    ...
    def enc_ht(self, head, tail, KG):
        ...
        hiddens = torch.FloatTensor(np.zeros((n_ent, len(head), self.n_dim))).to('cpu')
        hiddens[head, torch.arange(len(head)).to('cpu')] = head_embed
        ...
        hiddens = torch.FloatTensor(np.zeros((n_ent, len(head), self.n_dim))).to('cpu')
        hiddens[tail, torch.arange(len(tail)).to('cpu')] = tail_embed
        for l in range(self.L):
            hiddens = hiddens.view(n_ent, -1)
            relation_weight = self.attn_relation[l](nn.ReLU()(self.relation_linear[l](ht_embed)))
            relation_weight = torch.sigmoid(relation_weight).unsqueeze(2)
    ...
    def visualize(self, head, tail, edge_index, edge_weights, num_beam=5, head_batch=True):
        n_ent = self.all_ent
        inputs = torch.full((n_ent, num_beam), float('-inf')).to('cpu')
        if head_batch:
            inputs[head, 0] = 0
        else:
            inputs[tail, 0] = 0
        ...
        for i in range(len(edge_weights)):
            ...
            is_duplicate = is_duplicate.float() - torch.arange(num_beam, dtype=torch.float).to('cpu') / num_beam     # [n_edge, num_beam, num_beam]
            ...
            arange = torch.arange(len(sizes)).to('cpu')
            msg2out = arange.repeat_interleave(sizes)
            #msg2out= functional._size_to_index(size[node_out_set] * num_beam)     

            # deduplicate
            is_duplicate = (msg_source[1:] == msg_source[:-1]).all(dim=-1)
            is_duplicate = torch.cat([torch.zeros(1, dtype=torch.bool).to('cpu'), is_duplicate])
            ...
            if not torch.isinf(message).all():
                ...
                #print('second branch')
                distance = torch.full((n_ent, num_beam), float('-inf')).to('cpu')
                back_edge = torch.zeros(n_ent, num_beam, 4).long().to('cpu')
