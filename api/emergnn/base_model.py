...
class BaseModel(object):
    ...
    def train(self, train_pos, train_neg, KG):
        pos_head, pos_tail, pos_label = torch.LongTensor(train_pos[:,0]).to('cpu'), torch.LongTensor(train_pos[:,1]).to('cpu'), torch.FloatTensor(train_pos[:,2:]).to('cpu')
        neg_head, neg_tail, neg_label = torch.LongTensor(train_neg[:,0]).to('cpu'), torch.LongTensor(train_neg[:,1]).to('cpu'), torch.FloatTensor(train_neg[:,2:]).to('cpu')
        ...
        for p_h, p_t, p_r, n_h, n_t, n_r in tqdm(batch_by_size(n_batch, pos_head, pos_tail, pos_label, neg_head, neg_tail, neg_label, n_sample=n_train), 
            ...
            labels = torch.cat([torch.ones(len(p_scores)), torch.zeros(len(n_scores))], dim=0).to('cpu')
